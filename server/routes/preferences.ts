...
router.post('/:userId/preferences/ingredients', validateParamsAndBody(userIdSchema, ingredientPreferenceSchema), async (req, res, next) => {
    try {
        ...
        const ingredientResult = await db.query(
            'SELECT 1 FROM ingredients WHERE "ingredientId" = $1',
            [ingredientId]
        );
        ...
        const existingPref = await db.query(
            'SELECT 1 FROM "userIngredientPreferences" WHERE "userId" = $1 AND "ingredientId" = $2',
            [userId, ingredientId]
        );
        ...
        const result = await db.query(
            `INSERT INTO "userIngredientPreferences" ("userId", "ingredientId", "preference")
             VALUES ($1, $2, $3)
             RETURNING *`,
            [userId, ingredientId, preference]
        );
...
router.put('/:userId/preferences/ingredients/:ingredientId',
    validate(z.object({
        userId: userIdSchema.shape.userId,
        ingredientId: z.string().transform(val => parseInt(val, 10)).refine(val => !isNaN(val) && val > 0)
    }), 'params'),
    validate(z.object({ preference: z.enum(['like', 'dislike', 'stretch']) })),
    async (req, res, next) => {
        try {
            ...
            const result = await db.query(
                `UPDATE "userIngredientPreferences" 
                 SET "preference" = $1
                 WHERE "userId" = $2 AND "ingredientId" = $3
                 RETURNING *`,
                [preference, userId, ingredientId]
            );
...
router.delete('/:userId/preferences/ingredients/:ingredientId',
    validate(z.object({
        userId: userIdSchema.shape.userId,
        ingredientId: z.string().transform(val => parseInt(val, 10)).refine(val => !isNaN(val) && val > 0)
    }), 'params'),
    async (req, res, next) => {
        try {
            ...
            const result = await db.query(
                'DELETE FROM "userIngredientPreferences" WHERE "userId" = $1 AND "ingredientId" = $2 RETURNING *',
                [userId, ingredientId]
            );
...
router.post('/:userId/preferences/ingredients/bulk',
    validateParamsAndBody(userIdSchema, bulkIngredientPreferencesSchema),
    async (req, res, next) => {
        try {
            const { userId } = req.params;
            const { preferences } = req.body;
            ...
            try {
                ...
                for (const pref of preferences) {
                    // Check if ingredient exists
                    const ingredientExists = await db.query(
                        'SELECT 1 FROM ingredients WHERE "ingredientId" = $1',
                        [pref.ingredientId]
                    );
                    ...
                    const result = await db.query(
                        `INSERT INTO "userIngredientPreferences" ("userId", "ingredientId", "preference")
                         ...
                         RETURNING *`,
                        [userId, pref.ingredientId, pref.preference]
                    );
