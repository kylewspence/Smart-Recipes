...
export const requireOwnership = (resourceType) => {
    return async (req, res, next) => {
        try {
            ...
            switch (resourceType) {
                case 'recipe':
                    const recipeCheck = await db.query('SELECT "userId" FROM recipes WHERE id = $1', [resourceId]);
                    ...
                    if (recipeCheck.rows[0].userId !== userId) {
                        ...
                        const isPublic = await db.query('SELECT "isPublic" FROM recipes WHERE id = $1', [resourceId]);
 ...
 */
export const authRateLimitBypass = (req, res, next) => {
...
export const validateSession = async (req, res, next) => {
    try {
        ...
        const userCheck = await db.query('SELECT id, "isActive", "lastActiveAt" FROM users WHERE id = $1', [req.user.userId]);
        ...
        await db.query('UPDATE users SET "lastActiveAt" = NOW() WHERE id = $1', [req.user.userId]);
        next();
    }
    catch (error) {
        next(error);
 ...
 */
export const addAuthSecurityHeaders = (req, res, next) => {
    if (req.user) {
