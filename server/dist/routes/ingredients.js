...
router.get('/:ingredientId', async (req, res, next) => {
    try {
        ...
        const ingredientResult = await db.query(`SELECT i.*, 
                    COALESCE(
                        (SELECT COUNT(*)
                         FROM "recipeIngredients" ri
                         WHERE ri."ingredientId" = i."ingredientId"
...
router.post('/', async (req, res, next) => {
    try {
        ...
        const existingIngredient = await db.query('SELECT "ingredientId" FROM "ingredients" WHERE LOWER("name") = LOWER($1)', [ingredientData.name]);
        if (existingIngredient.rows.length > 0) {
            throw new ClientError(409, 'Ingredient already exists');
        }
        // Create the ingredient
        const result = await db.query('INSERT INTO "ingredients" ("name", "category") VALUES ($1, $2) RETURNING *', [ingredientData.name, ingredientData.category || 'other']);
        res.status(201).json({
            success: true,
            data: result.rows[0],
...
router.put('/:ingredientId', async (req, res, next) => {
    try {
        ...
        const existingIngredient = await db.query('SELECT * FROM "ingredients" WHERE "ingredientId" = $1', [ingredientId]);
        if (existingIngredient.rows.length === 0) {
            throw new ClientError(404, 'Ingredient not found');
        }
        // If updating name, check for duplicates
        if (updateData.name) {
            const duplicateCheck = await db.query('SELECT "ingredientId" FROM "ingredients" WHERE LOWER("name") = LOWER($1) AND "ingredientId" != $2', [updateData.name, ingredientId]);
        ...
        const result = await db.query(`UPDATE "ingredients" 
...
router.delete('/:ingredientId', async (req, res, next) => {
    try {
        ...
        const usageCheck = await db.query('SELECT COUNT(*) as count FROM "recipeIngredients" WHERE "ingredientId" = $1', [ingredientId]);
        ...
        await db.query('DELETE FROM "ingredients" WHERE "ingredientId" = $1', [ingredientId]);
        res.json({
            success: true,
...
router.put('/categories/bulk', async (req, res, next) => {
    try {
        ...
        const result = await db.query(`UPDATE "ingredients" 
             ...
             RETURNING "ingredientId", "name", "category"`, [newCategory, validIds]);
        res.json({
            success: true,
            data: {
